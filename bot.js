...
client.on("guildMemberAdd", async member => {
  const channel = member.guild.channels.find('name', 'hoşgeldin');//log ismini ayarlıyacaksınız log adında kanal açın
  if (!channel) return;
...
client.on('guildCreate', async guild => {
  const girismesaj = [
    ...
    'Bu bot **! LasTRomaNce#9390** tarafından geliştirilmektedir.',
    'r-yardım',
    'r-botbilgi',
    'r-ping',
    'r-davet ',
...
client.on('message', async msg => {
    ...
    .setTitle(' <:x:474973245477486612> | İlk olarak sesli bir kanala giriş yapmanız gerek.'));
      ...
      .setTitle(`**✅ | Oynatma Listesi: **${playlist.title}** Kuyruğa Eklendi!**`)
         ...
         .setTitle('Retdelta | Şarkı Seçimi')
   ...
   .setTitle(`:loud_sound: Şuanki Ses Seviyesi: **${serverQueue.volume}**`)
    ...
    .setColor('RANDOM'));
		  return msg.channel.sendEmbed(new Discord.RichEmbed()
    .setColor('RANDOM')
     .setTitle('Retdelta Müzik | Şarkı Kuyruğu')
...
function play(guild, song) {
  ...
  .setTitle("**Retdelta Müzik | 🎙 Müzik Başladı**",`https://cdn.discordapp.com/avatars/473974675194511361/6bb90de9efe9fb80081b185266bb94a6.png?size=2048`)
...
let prefix = "r-" //"!" yerine kendi botunuzun prefixini yazınız!
